...
sero_dat <- read_csv("generated_data/manitoba_pop_data_fake.csv") %>%
...
n_gva_age_cats <- nrow(gva_age_cats)

gva_age_cats <- bind_rows(gva_age_cats, gva_age_cats)
...
sero_dat$rha_name <- relevel(factor(sero_dat$rha_name), rha_name_ref)
...
gva_age_cats$rha_name <- relevel(factor(gva_age_cats$rha_name), rha_name_ref)
...
primary_dat$rha_name <- relevel(as.factor(primary_dat$rha_name), rha_name_ref)
...
primary_age_cats$rha_name <- relevel(as.factor(primary_age_cats$rha_name), rha_name_ref)
...
model <- stan_model("stan/serosurvey-analysis-re.stan")

X <- model.matrix(~Sex + age_cat, data = sero_dat)

result <- sampling(model, data = list(N_survey = nrow(sero_dat),
                                      H = max(sero_dat$household_id),
                                      R = max(sero_dat$rha),
                                      survey_pos = sero_dat$pos,
                                      hh = sero_dat$household_id,
                                      p_vars = ncol(X),
                                      ...
                                      rha = sero_dat$rha),
                    warmup = 500, iter = 2500, refresh = 100,
                    chains = 4, cores = 4, sample_file = "diag",
...
if (!file.exists(out_est_file) | params$redo) {
  ...
  re_seropos <- run_analysis_stan_re(
    ...
    dat = sero_dat,
    analysis = "ei",
    coef_eqn = "Sex + age_cat + week + rha_name",
