...
subset_est %>%
  filter(var == "Week_RHA") %>%
  separate(val, c("week", "rha_name"), convert = T) %>%
  group_by(sim) %>%
  mutate(coef_val = ifelse(week == 2 & rha_name == "WRHA", NA,
    ifelse(p > p[week == 2 & (rha_name == "WRHA")], 1, -1)
  )) %>%
  ungroup() %>%
  # mutate(week=relevel(val,"1")) %>%
  left_join(sero_dat %>%
    group_by(week, rha_name) %>%
    summarize(
      n = n(),
      pos = sum(pos),
      neg = sum(neg),
  ...
  group_by(week, rha_name, n, pos, neg, ind) %>%
  summarize(
    `Seroprevalence (95% CI)` = paste0(
      mean(100 * p) %>%
        formatC(2, format = "f"), " (",
  ...
  rename(
    Week = week, RHA = rha_name, Obs = n, `Test positive` = pos, `Test negative` = neg,
    `Indeterminate` = ind
  ) %>%
  knitr::kable(
    align = c("lcccccc"),
    caption = "Seroprevalence by week by RHA",
    booktabs = T, linesep = ""
  )
